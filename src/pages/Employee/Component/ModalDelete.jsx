...
import { softDeleteEmployee } from "../../../api/softDeleteEmployee"


export default function EmployeeModalDelete({ handleCloseModal, employeeData, isRefetch }) {
  const [isLoading, setIsLoading] = useState(false)
  
  const handleSoftDelete = async () => {
      setIsLoading(true)
      try {
        await softDeleteEmployee(employeeData.id)
  ...
  return (
    <Modal>
      ...
      <div className="text-center text-gray-400 mb-12">
        Mmenghapus Karyawan <strong>{employeeData.name}</strong> tidak dapat diulang
      </div>
