...
import ProcessedProductModalDelete from "../Component/ModalDelete";
...
export default function ProcessedProductIndex() {
  ...
  if (isLoading) {
    ...
    return (
      <>
        {productDataDelete && (<>
          <ProcessedProductModalDelete isRefetch={handleToggleRefetch} productData={productDataDelete} handleCloseModal={() => { handleToggleOpen(null) }} />
        </>
        )
        }
        <div>
                    ...
                    return (
                      <tr className="bg-white border-b border-gray-200 hover:bg-gray-50/50">
                        ...
                        <td className="px-6 py-4 text-right relative flex items-center gap-3 flex-wrap">
                          <button onClick={() => { navigate(`/processed-product/${val.id}`) }} className="px-3 py-2 border border-gray-300 rounded-xl flex gap-1.5 items-center group hover:bg-amber-50 hover:cursor-pointer transition-all hover:border-amber-500">
                            <IconEye className="group-hover:-translate-y-0.5 transition-all stroke-amber-500" stroke={1.2} size={22} />
                            Detail
                          </button>
                          <button onClick={() => { navigate(`/processed-product/edit/${val.id}`) }} className="px-3 py-2 border border-gray-300 rounded-xl flex gap-1.5 items-center group hover:bg-purple-50 hover:cursor-pointer transition-all hover:border-purple-500">
